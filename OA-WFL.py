...
#channel # channel response vector
#sigma # nosie power
#eta # de-nosing factor of receive
#p # transmit power of device


def weighted_averaging(w_list, num_sample_list, dev_ind, channel, p, eta, sigma):
    ...
    for k in keys:
        for i in range(len(w_list)):
            ind = dev_ind[i]
            w_avg[k] += num_sample_list[i] * w_list[i][k] * abs(channel[k]) * np.sqrt(p[k])

        w_avg[k] += torch.randn_like(w_avg[k]) * sigma  # Add noise after summation

        w_avg[k] /= np.sqrt(eta[k])  # Divide by the de-noising factor eta[k]
