...
def get_config(dataset_name):
    ...
    train_config = {
        'lr':2e-4, 
        'timesteps': 100,       # 500               # The number of timesteps to use when sampling.
        'epochs':100,          # 300, 1500              # rounds * local_epochs
        'rounds':20,
        'local_epochs': 5,
        'ema_decay': 0.998,                     
...
class CIFAR10Dataset(Dataset):
    def __init__(self, parent_dir, label_idx_dict=None, transform=None):
        ...
        self.classes = list(self.label_idx_dict.keys())  # To show what classes are in the dataset.

        for sub_dir in sub_dirs:
            full_path = os.path.join(parent_dir, sub_dir)
            img_paths = glob(os.path.join(full_path, '*.JPG')) + glob(os.path.join(full_path, '*.jpg')) + glob(
                os.path.join(full_path, '*.png'))
...
def generate_img(model, save_path, steps, eta, num_imgs, img_class, img_size=32, seed=42):
    ...
    with eval_mode(model):
        fakes = sample(model, noise, steps, eta, fakes_classes,start_step=20)
